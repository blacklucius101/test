...
bool GetBands(int count)
{
   int upper_count      = CopyBuffer(stoch_bands_handle, 0, 0, count, upper);
   int resistance_count = CopyBuffer(stoch_bands_handle, 1, 0, count, resistance);
   int mid_count        = CopyBuffer(stoch_bands_handle, 2, 0, count, mid);
   int support_count    = CopyBuffer(stoch_bands_handle, 3, 0, count, support);
   int lower_count      = CopyBuffer(stoch_bands_handle, 4, 0, count, lower);
   ...
   if(upper_count != count ||
      ...
      lower_count != count)
   {
      PrintFormat("Not enough band data. Error: %d", GetLastError());
      return false;
   }
   return true;
}
                ...
                const int &spread[])
{
   ...
   if(!GetBands(rates_total)) return 0;
   ...
   for(int i = start_idx; i < rates_total - 1; i++) {
      ...
      // bullish
		...
		{
			// countercross
			if(resStateL2.crossState == INT_SUP) // disagreeing border: special
			{
				if(IsBullishCandle(open, close, i) && open[i] < support[i]) // bullish cc (overhaul)
				{
			...
			{
				if(IsBullishCandle(open, close, i) && open[i] < mid[i] && close[i] > mid[i]) // bullish cc (countercross)
				{
			...
			if(resStateL2.crossState == INT_RES) // agreeing border
			{
				if(IsBullishCandle(open, close, i) && open[i] < resistance[i] && close[i] > resistance[i]) // bullish cc (countercross)
				{
			...
			// zonal crossState update
			if((open[i] > support[i] && close[i] < support[i]) || (open[i] < support[i] && close[i] < support[i]))
			{
		...
		{
			// countercross
			if(supStateL2.crossState == INT_RES) // disagreeing border: special
			{
				if(IsBearishCandle(open, close, i) && open[i] > resistance[i]) // bearish cc
				{
			...
			{
				if(IsBearishCandle(open, close, i-1) && open[i] > mid[i] && close[i] < mid[i]) // bearish cc (countercross)
				{
			...
			if(supStateL2.crossState == INT_SUP) // agreeing border
			{
				if(IsBearishCandle(open, close, i-1) && open[i] > support[i] && close[i] < support[i]) // bearish cc (countercross)
				{
			...
			// zonal crossState update
			if((open[i] < resistance[i] && close[i] > resistance[i]) || (open[i] > resistance[i] && close[i] > resistance[i]))
			{
